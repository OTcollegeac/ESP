...
def GetGoat():#This function reads the csv and determines the highest value (number of sales) a person has
    df=pd.read_csv('data.csv')
    df=ComputeTotalSales(df)
    print(f'The Best Sales Person of all time is {df["Total Sales"].idxmax()} with {df["Total Sales"].max()} Sales.')
...
def GetBestReigon(): #The following function was assisted by AI
    df=pd.read_csv('data.csv')
    df=ComputeTotalSales(df)
    regionTotals=df.groupby('Region')['Total Sales'].sum()
    print(f"The best performing region is {regionTotals.idxmax()} with {regionTotals.max()} total sales.")

def ComputeTotalSales(df):
    df['Total Sales']=df.iloc[:,4:].sum(axis=1)
    return df
